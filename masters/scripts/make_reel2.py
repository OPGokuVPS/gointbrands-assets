#!/usr/bin/env python3
"""Reel 2 — Conscious Wines sparkling, Veo 3.1 Lite image-to-video.
First frame = hosted reel2_frame.jpg (teal bottle on healthy brunch table).
Focus: vegan / low calorie / less sugar messaging.
Output: /root/gointbrands_assets/opts/reels_out/reel2_conscious.mp4
"""
...
body = {
    ...
    "prompt": (
        ...
        "bottle with the clean white CONSCIOUS label reading 'LOW CALORIE WINE', 'MADE WITH ORGANIC GRAPES', "
        "'91 CALORIES PER 250ML' and 'VEGAN'. A coupe of pale sparkling wine catches soft sunlight beside it, "
        ...
        "and '91 CALORIES'. Clean bright wellness aesthetic, shallow depth of field, premium low-calorie wine ad. "
...
jid=d0.get("id"); print(f"REEL2 submit id={jid} status={d0.get('status')}",flush=True)
done=poll(jid,"REEL2") if jid else {"status":"noid"}
if done and done.get("status") in ("completed","succeeded","done"):
    ...
    print("REEL2 saved",flush=True)
else:
    print(f"REEL2 not-done {done}",flush=True)
print("DONE_REEL2",flush=True)